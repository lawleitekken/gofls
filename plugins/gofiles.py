                    ...
                    if re.compile(rf'{doc_file_names}', re.IGNORECASE):
                        ...
                        try:
                            await client.copy_message(
                                ...
                                caption=messages.caption+Presets.CAPTION_TEXT_DOC.format(media_name, media_format, file_size)
                            )
                    ...
                    if re.compile(rf'{vid_file_names}', re.IGNORECASE):
                        ...
                        try:
                            await client.copy_message(
                                ...
                                from_chat_id=messages.chat.id,
                                message_id=messages.message_id,
                                caption=messages.caption+Presets.CAPTION_TEXT_VID.format(media_name, file_size)
                            )
                        except FloodWait as e:
                            time.sleep(e.x)
                        user_message[id] = message.message_id
